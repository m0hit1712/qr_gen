...
MAX_QR_SIZE_BYTES = 2900                          # Safe QR capacity (approx 3 KB)
MAX_FILE_SIZE_MB = 2                              # Maximum size per file (for sanity check)
...
def combine_data(image_path: str, json_path: str) -> bytes:
    ...
    return json.dumps(combined, separators=(",", ":")).encode()
    ...
    for dir_name in directories:
        ...
        try:
            ...
            if encoded_size > MAX_QR_SIZE_BYTES:
                logging.warning(f"{dir_name}: Encrypted data too large ({encoded_size} bytes) for a single QR, skipping.")
                continue
